WITH access_menu_seed (menu_key, label, path, display_order, permission_code) AS (
    VALUES ('admin-access-management', 'Access Management', '/admin/access-management', 18, 'MANAGE_USERS')
)
...
SELECT r.id, m.id
FROM auth.roles r
JOIN auth.menus m ON m.menu_key = 'admin-access-management'
