...
import com.nqh.inventoryservice.dtos.InternalProductOwnerResponse;
import com.nqh.inventoryservice.dtos.InternalProductOwnersLookupRequest;
...
public class InternalInventoryController {
    ...
    public ResponseEntity<BaseResponse<List<InternalProductOwnerResponse>>> getProductOwners(
            @RequestHeader(name = INTERNAL_TOKEN_HEADER, required = false) String internalToken,
            @RequestBody @Valid InternalProductOwnersLookupRequest request,
            HttpServletRequest httpServletRequest
    ) {
        validateInternalToken(internalToken);

        List<InternalProductOwnerResponse> response = request.getProductIds().stream()
                .distinct()
                .map(this::safeResolveOwner)
                .filter(owner -> owner != null && owner.getShopId() != null)
                .toList();

        return apiResponseFactory.success(HttpStatus.OK, MessageCode.COMMON_SUCCESS, response, httpServletRequest);
    }

    private InternalProductOwnerResponse safeResolveOwner(java.util.UUID productId) {
        try {
            java.util.UUID shopId = inventoryService.getProductOwnerShopId(productId);
            return InternalProductOwnerResponse.builder()
                    .productId(productId)
                    .shopId(shopId)
                    .build();
