...
function PartnerDashboardPage() {
  ...
  useEffect(() => {
    async function loadPartnerData() {
      if (!session?.userId) {
        setError('Không tìm thấy partner ID từ phiên đăng nhập.')
        setLoading(false)
        return
      }
      ...
      try {
        ...
        setError(extractApiErrorMessage(err, 'Không tải được Partner Dashboard.'))
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) {
    return <p className="role-muted">Đang tải Partner Dashboard...</p>
  }
    ...
    <section className="role-page-stack">
      ...
      <article className="role-card">
        ...
        <p className="role-muted">
          Dữ liệu hiển thị trong phạm vi partner hiện tại (theo userId đăng nhập).
        </p>
        <div className="role-metric-grid">
          <div className="role-metric-card">
            <span>Tổng đơn hàng Shopee</span>
          ...
          <div className="role-metric-card">
            <span>Tồn kho</span>
            <strong>N/A</strong>
            <small>Dùng trang Inventory để tra cứu theo productId.</small>
          </div>
        </div>
      </article>
    </section>
